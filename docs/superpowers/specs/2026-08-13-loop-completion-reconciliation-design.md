...
de promoção cujo commit não pertence ao histórico do branch atual. O registro
continuará preservado para diagnóstico, mas só bloqueará uma candidata quando o
commit promovido estiver alcançável a partir de `HEAD`.
...
2. Considerar concluído somente o registro cujo campo
   `promoted_commit` seja uma string não vazia e cujo commit seja ancestral de
   `HEAD` no repositório do projeto.
...
4. Consultar Git com argumentos separados, sem shell, e tratar qualquer falha
   de `git merge-base --is-ancestor` como registro não concluído.
...
parcialmente ou em outro branch.
