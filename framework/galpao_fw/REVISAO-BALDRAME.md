...
> **STATUS: ✅ HOMOLOGADO (2026-07-08)** — correções de cisalhamento e armadura
> superior aplicadas conforme parecer sênior. `_verifica_cortante()` adicionada;
> `As_sup` corrigida para armadura construtiva (2 φ 6,3 mm porta-estribos).
...
- **Composição:** `As,inf = max(As,flexão + As,tie/2 ; As,min)` ;
  `As,sup = max(As,tie/2 ; As_construtiva)`. **As,min (ρmin) NÃO se aplica à face
  comprimida** (Tabela 17.3 é para a face tracionada). Na face superior sob
  compressão usa-se apenas armadura construtiva **2 φ 6,3 mm porta-estribos**
  (≈ 0,62 cm²). Barras por `detalha_barras`.
- **Cortante (17.4.2 Modelo I):** verificação explícita da biela comprimida
  (`VRd2 = 0,27·αv2·fcd·b·d`, `αv2=1−fck/250`) e da armadura mínima
  (`VRd3_min = Vc + Vsw_min`). `Vd = γf·w·L/2`. Antes o cisalhamento era assumido
  sem cálculo — agora é verificado obrigatoriamente e integra o `OK`.
- **Detalhamento:** largura mínima **b ≥ 12 cm** (NBR 6118 **13.2.2**, verbatim:
  *"a seção transversal das vigas não pode apresentar largura menor que 12 cm"*);
  ...
  se `Vd > 0,67·VRd2` → `s_max = 0,3·d ≤ 200 mm` (**18.3.3.2**).
...
3. ~~**Não verifica cisalhamento**~~ — **RESOLVIDO.** `_verifica_cortante()` adicionada:
   calcula `VRd2` (biela, 17.4.2.2) e `VRd3_min` (estribo mínimo, Eq. 17.17).
   A verificação integra o `OK` da viga. Ref 20×10: `Vd=28 kN`, `VRd2=291 kN`,
   `VRd3_min=79 kN` → u≪1, folgado.
...
| Cortante (VRd2/VRd3) | `_verifica_cortante` | 17.4.2 / Eq.17.10/17.17 |
| Largura mínima | `verifica_baldrame` | 13.2.2 |
| Estribo s_max | `verifica_baldrame` | 18.3.3.2 |
| Detalhe de barras | `fs.detalha_barras` | 18.3 |
| Armadura construtiva superior | `AS_CONSTRUTIVA_SUP` | 17.3.5.2.1 (face tracionada apenas) |
...
conferida; domínio; cortante verificado; As_sup < As_min (construtiva, não
antieconômico); b<12 reprova; seção pequena sob M grande reprova). Orquestrador:
`N_amarracao` auto = **max|V| da base no envelope** (ref: 32,9 kN do
C2_uplift_W2), `vao` = bay; `gate7-baldrame.txt`, item **11b** no consolidado.
Ref 20×10 (fechamento telha, `q_parede=0`): baldrame 20×40, As,inf=3,02 cm²
(2Φ16), As,sup=0,62 cm² (2φ6,3 porta-estribos), Vd=28 kN, VRd2=291 kN,
VRd3_min=79 kN. **APROVADA**. Aditivo: só roda com `params["baldrame"]`.
