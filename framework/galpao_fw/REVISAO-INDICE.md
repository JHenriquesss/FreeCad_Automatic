...
| 46 | DG25 envelope — FLB (§5.4.4), TFY (§5.4.5), ruptura (§5.4.6), envelope min (§5.4.7) | [REVISAO-DG25-ENVELOPE.md](REVISAO-DG25-ENVELOPE.md) | AISC Design Guide 25 §5.4.4–5.4.7 (5.4-14..5.4-32; pág 62–64 verbatim por imagem) | ✅ PARECER 2 RODADAS — 2 CORRIGIDOS (fase 6.16, 2026-07-13): F1 kc 5.4-24 hc→**hw**; F2 teto Mp do Rpt Sxt→**Sxc** (5.4-28 imprime Sxt = erratum; remete ao Rpc pág 60 que usa Sxc). λ do Rpt = hc/tw conferido. 13 testes |
| 47 | Forças transversais localizadas + enrijecedor de apoio | [REVISAO-FORCAS-LOCALIZADAS.md](REVISAO-FORCAS-LOCALIZADAS.md) | NBR 8800 §5.7.2–5.7.9 (6,25tf²; 1,10(5k+ln); enrugamento 0,66/0,33; Cr[0,94+0,37]; 24tw³; enrij. Lb=0,75h faixa 12tw/25tw; pág 57–62 verbatim por imagem) | ✅ HOMOLOGADO SEM IMPEDITIVOS (fase 6.17, 2026-07-13): 4 contas manuais auditadas (eng. reversa k=20/ln=100 batem casas decimais); R1 k/ln documentados no teste; R2 inércia I_par aceita (simplif. ~0,05%). 11 testes |
| 48 | Viga de equilíbrio de divisa sobre estacas (variante profunda) | [REVISAO-VIGA-EQUILIBRIO.md](REVISAO-VIGA-EQUILIBRIO.md) | Alonso/Velloso & Lopes (R'=P·l/(l−e), estática já validada em `sapata_divisa`) + estaca_profunda + NBR 6118 (viga RC) | ✅ HOMOLOGADO — 3 CORRIGIDOS + pele (fase 6.18, 2026-07-13): (1) M do fletor **R'·e→P·e** (erro de estática); (2) cisalhamento NBR 6118 §17.4 (biela VRd2+estribo, V=ΔP); (3) peso próprio ~5% nas estacas; (4) 2ª rodada: armadura de pele §17.3.5.2.3 (h>60cm). 13 testes |
| 49 | Executivo 2D — glyph AWS de solda (headless via DrawViewSymbol) + quadros PE09 legíveis | [REVISAO-EXECUTIVO-POLISH.md](REVISAO-EXECUTIVO-POLISH.md) | AWS A2.4 (símbolo de filete) + TechDraw | ✅ PARECER — 1 CORRIGIDO (fases 6.19/6.12, 2026-07-13): glyph deixou de ser arrow-side engessado; parametrizado **arrow/other/both** (AWS A2.4, posição do triângulo = lado da junta), lido do dado de fabricação. PE09 cosmético aprovado. 9 testes |
...
| [G14-GESTAO-EDIFICIO](REVISAO-G14-GESTAO-EDIFICIO.md) | O edifício ganha orçamento/cronograma/caderno/pacote (item aberto pelo próprio G7). O mecanismo era o do G6; o que faltava era a DERIVAÇÃO de quantitativos de um prédio de concreto. Três guardas contra o orçamento parcial que se diz fechado: armadura por ELEMENTO (a de viga sai vazia — as vigas do edifício são analisadas e nunca verificadas), `aplicaveis` separando "a obra não tem" de "ninguém quantificou", e os insumos sem preço na tabela nomeados. Achados: o prédio do spec não tem fachada declarada, e o índice de pranchas do pacote listava 13 folhas ao lado de uma pasta com 2 |

| [G18-VIGA-BALDRAME-RECALQUE](REVISAO-G18-VIGA-BALDRAME-RECALQUE.md) | Os dois últimos itens de escopo de fundação saem de `not_available` quando declarados: viga baldrame (fronteira para `viga_baldrame.py`, reações por viga contínua) e recalque diferencial (fronteira para `geotecnia_spt`/`estaca_profunda`, Es do laudo ou nada). Sem parede declarada / sem Es não há cálculo — entrada arbitrada é bug, não default |
| [G20-MEZANINO-CONCRETO-METALICO](REVISAO-G20-MEZANINO-CONCRETO-METALICO.md) | Primeira costura concreto × metálico em uso real: mezanino (laje+viga+pilar+sapata, NBR 6118) DENTRO do envelope do galpão validado, nas mesmas coordenadas do `modelo_neutro` (federado sem transformação). Três fronteiras novas F18 (geometria m) / F19 (laje h cm com realimentação) / F20 (seção m). 1 painel só; sem porta, sem viga intermediária, sem chumbamento inventado |
| [G22-FRONTEIRAS-COPIA-ISOLADA](REVISAO-G22-FRONTEIRAS-COPIA-ISOLADA.md) | Higiene da prova do G21: as 3 mutações do G8 passam a rodar numa CÓPIA do pacote em diretório temporário (mesmo mecanismo, isolamento por cópia). Elimina a classe de acidente "processo morre na janela de mutação e o repo fica com dois módulos quebrados" |
| [G23-ALCANCE-MOMENTO-G19](REVISAO-G23-ALCANCE-MOMENTO-G19.md) | Auditoria de alcance do M_base do G17: cada consumidor (sapata, bloco, estaca, divisa, equilíbrio, BIM, orçamento, gestão, turnkey) USA ou IGNORA com razão declarada — divisa ignora M (P·e domina 20-50×) sem inventar modelo. Item pequeno: `[INFRA]` no NOME do check separa falha de caminho de divergência numérica. G19 cabeado sem destravar: 9 campos seguem pendentes, proposta 36×24 é hipótese nos 4 lugares |

| [G28-FUNDACAO-FONTE-BLOQUEADA](REVISAO-G28-FUNDACAO-FONTE-BLOQUEADA.md) | **BLOQUEADO por fonte — mesmo precedente honesto da alvenaria (NBR 16868).** Vertical de fundação (G9/G17/G18) é o único sem caso externo com laudo SPT: Petrópolis só tem `Sondagem 3×10 m` como item de serviço, UFPE/25×54 só perfis de aço. Caça FNDE 2026-09-03: padrão é *Fundação Típica hipotética* + item `SDG` NBR 6484/8036; laudo SPT é anexo municipal (Leopoldina ETP), não peça central replicável — laudos dispersos (GO 2 furos, Boa Vista 3 furos) existem mas sem pacote obra FNDE completo. Até que um pacote FNDE com laudo anexo seja coletado via `extrai_fonte_externa`, o vertical fica sem validação externa **com motivo escrito** (o que G28 proíbe é ficar sem caso e sem registro) |
Última atualização: 2026-07-08 (matemática) · 2026-08-30 (arquitetura, G8-G9, G11, G14) · 2026-09-03 (G28 bloqueio fundação) · 2026-09-04 (G36: G18/G20/G22/G23 + aviso circularidade demo_g19).
