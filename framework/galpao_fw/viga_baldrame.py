...
def verifica_baldrame(cfg):
    ...
    As_sup = max(As_tie / 2.0, As_min)             # face superior: amarracao + min
    arr_inf = fs.detalha_barras(As_inf, b, cob)
    arr_sup = fs.detalha_barras(As_sup, b, cob)

    # ---- detalhamento (NBR 6118 13.2.2 / 18.3.3.2) ------------------------
    s_estribo = min(0.6 * d, 0.30)                 # V<=0,67 VRd2 (governa neste porte)
    b_ok = b >= B_MIN - 1e-9

    OK = sec_ok and ok_dom and b_ok
    return {"vao": L, "b": b, "h": h, "d": round(d, 3), "M_d": round(M_d, 2),
            ...
            "ok_dominio": ok_dom, "sec_ok": sec_ok, "b_ok": b_ok,
...
def relatorio_pt(r):
    ...
    L = ["VIGA DE BALDRAME / AMARRACAO (ABNT NBR 6118:2014)",
         ...
         f"  Estribos: phi {r['phi_estribo_mm']:.1f} mm ; s_max = 0,6d <= 300 mm = "
...
def _selftest():
    ...
    # domínio ok e secao suficiente
    assert r["ok_dominio"] and r["sec_ok"] and r["b_ok"] and r["OK"], r
