...
## T40 — Sessão 40 (2026-08-03): dupla-conversão de janela — ABERTO (bug real, não corrigido)
Descoberto na varredura de saturação. `aberturas["janelas_laterais"]` tem **duas convenções conflitantes** no código: **(L,H)** (dims do usuário) vs **faixa (z_base,z_topo)** (o que o build 3D espera). O PR #144 (S39) fez `wizard.construir_spec` converter (L,H)→faixa e gravar no spec — MAS `to_build_kwargs`→`aberturas_para_build`→`_janela_band` (projeto_spec) **ainda reconverte**, reinterpretando a faixa pronta como (L,H): wizard produz (1000,2000) → `_janela_band` → (1100,3100). Janela errada no build (o IFC, que lê `spec.get("aberturas")` cru em `ifc_emit`, fica CERTO). **Testes pinam AS DUAS convenções** (`test_wizard_robustez` exige spec=faixa; `test_aberturas_janela::test_portao_permanece_LH` exige `aberturas_para_build` converter; `test_ifc_emit`/`test_modelo_neutro` passam (3000,1000) descendente). **Fix correto = unificar a convenção projeto-inteiro** (canônica = FAIXA, que build/ifc/neutro/defaults já usam): wizard produz faixa (já faz), REMOVER a reconversão em `aberturas_para_build`, e arrumar os testes que pinam (L,H). Até lá, `test_aberturas_janela::test_to_build_kwargs_converte_janela_do_wizard` segue **VERMELHO** — falha PRÉ-EXISTENTE (já falhava em `main` antes da S40), não regressão. Detalhe completo em `memory/janela-dupla-conversao-aberta.md`.
